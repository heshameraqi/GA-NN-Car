...
%         axis([0 -120 0 160]);
    ...
    for car_id = 1:length(collision_bools)
        ...
        Fitness = LifeTimes(car_id) - sqrt((carLocations(car_id,1)-env.destination(1))^2+(carLocations(car_id,2)-env.destination(2))^2);
        ...
        %         axis([0 -120 0 160]);
                ...
                % Handle if destination is reached
                if ( sqrt((carLocations(car_id,1)-env.destination(1))^2+(carLocations(car_id,2)-env.destination(2))^2) <= 3)
                    return;
                end
